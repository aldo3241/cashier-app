    ...
    <script>
        ...
        function updateLanguage() {
            ...
            if (modal) {
                ...
                modalElements.forEach(element => {
                    // Skip elements that have been updated with real data (contain "Rp" or specific data)
                    if (element.id === 'masuk-amount' && element.textContent.includes('Rp')) {
                        return; // Skip this element as it has real data
                    }
                    if (element.id === 'payment-method' && !element.textContent.includes('Memuat') && !element.textContent.includes('Loading')) {
                        return; // Skip this element as it has real data
                    }
        ...
        $(document).ready(function() {
            ...
            var table = $('#salesTable').DataTable({
                responsive: true,
                pageLength: 50, // Show 50 records per page by default
                lengthMenu: [[25, 50, 100, 200, -1], [25, 50, 100, 200, "All"]], // Length menu options
                ...
                dom: 'lfrtip', // Include length menu for user control
        ...
        async function loadTransactionDetails(transactionId, invoiceNumber) {
            try {
                console.log('Loading transaction details for invoice:', invoiceNumber);
                ...
                console.log('Keuangan response status:', keuanganResponse.status);
                
                if (keuanganResponse.ok) {
                    const keuanganData = await keuanganResponse.json();
                    console.log('Keuangan data:', keuanganData);
                    
                    if (keuanganData.success) {
                        document.getElementById('masuk-amount').textContent = 'Rp ' + new Intl.NumberFormat('id-ID').format(keuanganData.data.masuk);
                        ...
                        document.getElementById('payment-method').textContent = keuanganData.data.keuangan_kotak;
                        
                        // Note: Language is already set when modal is created, no need to update
                    } else {
                        console.log('Keuangan API error:', keuanganData.message);
                        document.getElementById('masuk-amount').textContent = 'Error loading data';
                        document.getElementById('payment-method').textContent = 'Error loading data';
                    }
                } else {
                    console.log('Keuangan API failed with status:', keuanganResponse.status);
                    document.getElementById('masuk-amount').textContent = 'API Error';
                    document.getElementById('payment-method').textContent = 'API Error';
                }
                ...
                document.getElementById('masuk-amount').textContent = 'Error loading data';
                document.getElementById('stock-mutations-list').innerHTML = '<p class="text-red-500" data-en="Error loading data" data-id="Error memuat data">Error loading data</p>';
            }
        }
